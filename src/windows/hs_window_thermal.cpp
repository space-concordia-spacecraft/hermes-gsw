...
namespace hermes {
    ...
    void WindowThermal::RenderGUI() {
        ...
        if (ImPlot::BeginPlot("##Scrolling", ImVec2(-1, 150))) {
            ...
            ImPlot::PlotShaded("Cell #1", &sdata1.Data[0].x, &sdata1.Data[0].y, sdata1.Data.size(), -INFINITY,
                               sdata1.Offset, 2 * sizeof(float));
            ImPlot::PlotLine("Cell #2", &sdata2.Data[0].x, &sdata2.Data[0].y, sdata2.Data.size(), sdata2.Offset,
                             2 * sizeof(float));
            ImPlot::EndPlot();
        }
        ...
        ImGui::Text("%s", "\nTHERMAL OVERVIEW ");
        if (ImGui::BeginTable("table1", 3, 16)) {
            ImGui::TableSetupColumn("ELEC", 32);
            ImGui::TableSetupColumn("ADCS", 32);
            ImGui::TableSetupColumn("CDH", 32);
        ...
        for (int row = 0; row < 8; row++) {
            ...
            for (int column = 0; column < 3; column++) {
                if (column < 1) {
                    ...
                    ImGui::Text("%s %d %s", mElec[row], 0, "°C");
                } else if (column > 0 && column < 2) {
                    if (row < 6) {
                        ImGui::TableSetColumnIndex(column);
                        ImGui::Text("%s %d %s", mADCS[row], 0,"°C");
                    }

                } else if (column > 1 && column < 3) {
                    if (row < 3) {
                        ImGui::TableSetColumnIndex(column);
                        ImGui::Text("%s %d %s", mCDH[row], 0, "°C");
                    }
        ...
        if (ImGui::BeginTable("table2", 2, 16)) {
            ImGui::TableSetupColumn("TT&C", 32);
            ImGui::TableSetupColumn("Payload", 32);
            ImGui::TableHeadersRow();
        }
        for (int row = 0; row < 5; row++) {
            ...
            for (int column = 0; column < 3; column++) {
                if (column < 1) {
                    if (row < 2) {
                        ImGui::TableSetColumnIndex(column);
                        ImGui::Text("%s %d %s", mTTCC[row], 0, "°C");
                    }
                } else if (column > 0 && column < 2) {
                    if (row < 1) {
                        ImGui::TableSetColumnIndex(column);
                        ImGui::Text("%s %d %s", mPayload[row], 0, "°C");
                    }
